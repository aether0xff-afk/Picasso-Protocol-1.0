...
def main() -> None:
    parser = argparse.ArgumentParser(description="Split a PicassoBrain V6 bundle")
    parser.add_argument(
        "--bundle",
        type=Path,
        default=Path(__file__).parent / "brain_v6" / "picasso_brain_v6_bundle.pt",
    )
    parser.add_argument("--public-dir", type=Path, default=ROOT / "models" / "public_v6")
    parser.add_argument("--private-dir", type=Path, default=ROOT / "models" / "private_v6")
    ...
    if set(public_checkpoint) != {"config", "public_fragment"}:
        raise RuntimeError("public V6 export contains private brain data")
